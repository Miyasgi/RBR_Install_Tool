...
if (-not $qbtExe) {
    ...
    Write-Host "  https://www.qbittorrent.org/download" -ForegroundColor Cyan
    Write-Host ""
    if ($GuiMode) {
        Write-Log -Level "INFO" -Message "qBittorrent not found in GuiMode, opening download page automatically."
        try {
            Start-Process "https://www.qbittorrent.org/download"
            Write-OK "已自动打开 qBittorrent 下载页"
        } catch {
            Write-Warn "自动打开下载页失败，请手动访问：https://www.qbittorrent.org/download"
        }
    } else {
